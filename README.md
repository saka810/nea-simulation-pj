...
リポジトリ直下の **`シミュレーション起動.bat`** をダブルクリックする。
`.venv` の Python で `geosim/app.py` を開くところまでやってくれるので、
PowerShell を開く必要はない。

- **プロジェクトフォルダをこの .bat の上にドラッグ＆ドロップ**すると、
  そのプロジェクトを開いた状態で始まる（`python app.py "<フォルダ>"` と同じ）
- **黒い画面（実行ログ）は閉じないこと**。計算の進み具合と警告がここに出る
- ★**直下に置いたまま使う**（自分の場所を基準に `.venv` と `geosim` を探すので、
  デスクトップなどへコピーすると動かない。ショートカットを作るのは可）
- `.venv` が無ければ作り方を表示して止まる（端末ごとに作り直すもの）
...
5. **可聴化**（「可聴化…」または `python geosim/auralize.py <フォルダ>`）…
   インパルス応答をドライソース（`ドライソース/` の wav。★Git には入っていないので、
   端末ごとに `ドライソース/README.md` の手順で OneDrive からコピーする。★**自分の音源はこのフォルダに置くか
   画面へ落とせば一覧に並ぶ**）かマイク入力に畳み込んで聴く。
   ブラウザの窓で開き、**再生を止めずに**条件・受音点・響きの ON/OFF を切り替えられる。
   波形をドラッグで範囲選択 → ループ、クリックでその位置から再生。
   条件どうしの**音量の比は保つ**（吸音を増やすと静かになるのがそのまま聞こえる）
...
逆二乗.csv/png        ★別ルート：`python geosim/inverse_square.py <フォルダ>`
                      測線（rec1/rec2/…）ごとに理想の 1/r² からのずれを出す
                      （半無響室・無響室の確認用。ふだんの出力には入れない）
視点.json             画角（見る向き）★条件を変えても同じ向きで見比べるため
                      画面左の「画角を保存」「画角を読込」で出し入れする
                      名前を付けて何本でも入る（保存・読込とも名前を聞く窓が出る）
結果/                 ← 受音点に依らないものは直下
...
```
シート「吸音率」   番号・材料名・吸音率・種類（★PJ 固有の吸音データをここに置ける）
                   1000 材料ぶんの枠。種類はドロップダウンで選ぶ
シート「現状」     番号 / 区分 / レイヤー名 / 材料番号 / 安全率 / 材料名（参考）
                   / 面数 / 面積_m2      30 レイヤぶんの枠
```

**書き換えるのは「材料番号」と「安全率」の列だけ**（材料名は VLOOKUP で自動表示。
名前を打つと全角・半角のミスタイプが起きるため）。**条件シートに吸音率は載せない**
（ミスリードを避けるため。吸音率は「吸音率」シートにだけ置く）。

**安全率**は「吸音率を見過ぎないように間引く」ための係数（例 0.8）。
未入力なら掛けない。カタログ値に掛けてから垂直入射へ変換する。

表示は**吸音率・安全率が小数 2 桁、面積が小数 1 桁**。作った表の体裁
（列幅・書式・足した列）は**計算しても壊れない**（その場で直すだけ）。
