...
**複数条件は一括で回せる。** 条件表を `現状.csv` `吸音追加案.csv` … と名前を付けて
同じフォルダに並べ、条件入力の「全条件を一括 ▶▶」を押す（CLI なら
`python geosimun_project.py "<プロジェクト>" --conditions`）。

- **結果ファイル名は「DXF のファイル名 ＋ 条件表のファイル名」**になるので混ざらない
...
**441 項目**（2026-08-21 時点）。解析的に答えが分かる問題（直方体の虚音源距離、
