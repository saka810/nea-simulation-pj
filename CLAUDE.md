...
- `tests\test_geosim.py` … 数値検証（323 項目）。pytest 不要、素の Python で走る。
...
- **全測定点のまとめ表を作る**（`summary.py`。`結果/<室>_まとめ_残響時間.csv` /
  `<室>_まとめ_明瞭度.csv`）。計算後に自動で作られる。残響時間には**理論値も同じ表に**入れる。
  作り直しは `python summary.py <プロジェクト>`（計算はやり直さない）
