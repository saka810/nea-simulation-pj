...
- `TODO.md` … 作業一覧（A〜F にグルーピング）。着手・完了したらチェックボックスを更新する。
- `docs\技術説明書.md` … プログラムのフローと数式の意味の解説資料。
  数式の根拠は書籍『建築音響物理学』2.2 節。**数式に関わるコードを変更したらこの文書も更新する。**
- `参考文献\` … 論文・書籍の PDF。**PDF 本体は `.gitignore` で除外**（このリポジトリは public のため）。
  フォルダ構成と文献一覧（`参考文献\README.md`）だけを共有する。PDF の受け渡しは OneDrive 経由。
