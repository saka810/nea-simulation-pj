...
- `tests\test_geosim.py` … 数値検証（464 項目）。pytest 不要、素の Python で走る。
...
- **吸音材は「条件表」（xlsx）で決める**（`condition_table.py`。2026-08-21 ユーザー要望）。
  ...
  - **シート「吸音率」** … PJ で使う材料の一覧（番号・材料名・α・種類）。
  ...
  - ★**入力は「材料番号」の列だけ**。材料名は全角・半角のミスタイプを誘発するので
    使わない（ユーザー指摘）。材料名の列は `VLOOKUP` の**参考**で、読むときは見ない
