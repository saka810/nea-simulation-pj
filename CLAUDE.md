...
- `tests\test_geosim.py` … 数値検証（441 項目）。pytest 不要、素の Python で走る。
...
- **結果ファイル名の頭に「対象室名＋条件名」を付ける**（2026-08-21 ユーザー要望）。
  **名前はファイル名から取る**：対象室名＝**DXF のファイル名**（物件名・部屋名）、
  条件名＝**材料条件表のファイル名**（`Project.room_label` / `condition_label`）。
...
- **条件（吸音材）はファイルで切り替え、一括で回せる**（2026-08-21 ユーザー要望）。
  同じフォルダに `現状.csv` `吸音追加案.csv` … と条件表を並べ、条件入力で選ぶ。
