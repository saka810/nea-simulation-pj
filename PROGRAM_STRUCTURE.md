...
| 決めごと | 中身 | 理由 |
|---|---|---|
| ★★**音量の比を保つ** | 全結果でいちばんエネルギーの大きい応答を 1 とする**共通の 1 つの係数**（`Catalog.gain`）だけを掛ける。ブラウザ側も `disableNormalization: true` | 応答は `振幅 = √E/(t·c)` で絶対値を持つ。条件ごとに正規化すると「吸音を増やすと静かになる」「遠い受音点は小さい」が聞こえなくなる |
...
音源位置と受音点はフォルダ名。カードの T30 / EDT は同じ場所の `rt.csv` の中音域（500・1k Hz）の平均、
小さな減衰曲線は全帯域のシュレーダー積分（サーバで 96 点に間引く）。
