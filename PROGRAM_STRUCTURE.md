...
| Eyring-Knudsen | `-S·ln(1-ᾱ) + 4mV` | 吸音率が大きいとき。**実務の主役** |
| Millington | `-Σ Sᵢ·ln(1-αᵢ) + 4mV` | 参考値。αᵢ→1 の面があると T→0 になる |

- ★**名前**：アイリングの式は `-S ln(1-ᾱ)` までで、**空気吸収 `4mV` を足した形は
  ヌードセンの寄与**なので `アイリング・ヌードセン` と呼ぶ（ユーザー指摘 2026-08-17。
  それまで空気吸収込みなのに `Eyring` と表示していた）。
  表示名は `statistical_labels()` が `include_air_absorption` を見て切り替える
- ★**Millington は参考値**。ミリントン・セッテの式（1932/1933）。面ごとに対数を取るので、
  **α→1 の面が 1 枚でもあると A が発散して T→0** になる（開口が 1 つで残響ゼロ）。
  研修室（吸音面 α=0.951）では中高域で Eyring-Knudsen の約半分に出た
