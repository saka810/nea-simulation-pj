...
| `dxf_faces.py` | **ACIS（REGION / 3DSOLID）の DXF を面の DXF に直す**。accoreconsole で EXPLODE し、輪郭を組み直す。★出力は **CP932** で読む（日本語の画層名が壊れていた）／★**穴のある面はキーホール法で穴を開けたまま三角形に割る**（2026-09-09） | （移植元なし・新規） | **実装済**（2026-08-28 新設。B-22） |
