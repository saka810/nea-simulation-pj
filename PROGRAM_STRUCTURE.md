...
| `view_field.py` | **音圧分布を 3D 画面のタブで見る**（`mode_shape.py` の CSV を面に貼る） | （移植元なし・新規） | **実装済**（2026-08-26 新設） |
| `open_edges.py` | **開いた辺（自由端 / T字接合）を確かめる**。表・図・3D に重ねる線 | （移植元なし・新規） | **実装済**（2026-08-28 新設） |
| `dxf_faces.py` | **ACIS（REGION / 3DSOLID）の DXF を面の DXF に直す**。accoreconsole で EXPLODE し、輪郭を組み直す。★出力は **CP932** で読む（日本語の画層名が壊れていた）／★**穴のある面はキーホール法で穴を開けたまま三角形に割る**（2026-09-09）／★**元の図面の `POINT`（音源・受音点）を引き継ぐ**（2026-09-11） | （移植元なし・新規） | **実装済**（2026-08-28 新設。B-22） |
| `source_mix.py` | **音源が複数のときの結果の見方**（平均／重ね合わせ 2 通り）。★計算はやり直さない | （移植元なし・新規） | **実装済**（2026-09-15 新設。不具合報告 ⑨） |
| `point_order.py` | **測定点（音源・受音点）の並び**（`測定点順.json`）。`結果/recN/` の N を CAD のラベルに合わせる | （移植元なし・新規） | **実装済**（2026-09-15 新設） |
| `section.py` | **断面の決め方**（`断面.json`）。軸に平行な面と、平面図の測線を含む鉛直面 | （移植元なし・新規） | **実装済**（2026-08-27 新設） |
...
| `auralize.py` ＋ `auralize.html` | **可聴化**。インパルス応答をドライソース（またはマイク入力）に**リアルタイムで**畳み込んで聴く。画面はブラウザ、音は Web Audio の `ConvolverNode`。★計算はやり直さない | （移植元なし・新規） | **実装済**（2026-09-26 新設） |
...
#### ★★剛な面（α_s = 0）は NaN にしない（2026-09-16。ユーザー指摘で修正）

`α_s = 0` は **z = ∞**（完全に剛）に対応する。式をそのまま書くと

    α_n = 4z/(1+z)²  →  4·∞/∞² = NaN

になり、**剛な面を 1 つ指定しただけで結果が黙って全部 NaN になった**
（警告も出ないので気づけない）。`frequency_response.py` は
「まずは剛な面としてモード分布を確認する」使い方を想定しているので実害があった。

**極限を入れて直した**（`absorption.py`）。どれも数学的な極限そのもの。

| 関数 | z = 0 | z = ∞ | 意味 |
|---|---|---|---|
| `normal_absorption` | 0 | **0** | どちらも全反射（吸音しない） |
| `statistical_absorption_closed_form` | 0 | **0** | 同上 |
| `reflection_coefficient` | −1 | **+1** | 剛なら同位相で全反射 |

★あわせて **`random_to_normal` に見張りを置いた**。結果に NaN/inf が混じったら
**理由を告げて止める**（黙って計算へ流さない）。同じ種類の不具合が再発しても
「動いているつもりで結果が壊れている」状態にはならない。

検算：`α_s = 0 → α_n = 0 → |R|² = 1`（全反射）。拡散入射で平均し直すと
ちょうど 0 に戻る。床天井だけ吸音・壁は完全に剛の箱で通し計算が回り、
T30 は理論の **3.35 倍**（壁を 0.01 にした近似の 3.29 倍と整合）。
...
★★**三角形の辺は描かない。計算で使う同一平面パッチの外周だけを描く**（2026-09-24
ユーザー指摘「計算上三角形要素で見てないのであれば，三角形要素の表示はやめてほしい。
計算をこのモデルで行っていると勘違いしてしまいます」）。パッチの割り方は
**本計算と同じ `mesh_method.coplanar_patches`**（同一平面＋連結＋同じ材料＋同じ向き）。
法線の矢印もパッチごとに 1 本（根元はいちばん大きい三角形の重心。パッチの重心は
L 字・ドーナツで面の外に出る）。レイヤ欄の数と概要も**面（パッチ）の数**にして、
...
中身の大きさ＝キャンバスの実寸になるので、**ウィンドウを狭めても縮めず**、
右の操作パネル（幅 300 px）を画面の外へ押し出していた
（広い画面で開いてから狭い画面へ移す・ウィンドウを縮めると起きる。
表示倍率が 100% を超えると実寸が CSS の幅より大きくなるので確実に起きる）。
...
| 関数 | 対応元コード | 処理 |
|---|---|---|
| `third_octave_bands()` | 46 行 | 1/3 オクターブ 32 バンドの**厳密**中心周波数（呼び値 16 Hz〜20 kHz ＝ 15.849〜19 953 Hz。2026-09-16 にベース 10 へ） |
...
| バンド端 | `(mf*2)^(-1/6)`、指数に fmax が混入 | `mf * G^(∓1/6) / fmax`（G = 10^(3/10)。`absorption.band_edges`） |
...
★★**`decay_floor_db` は「崖の手前」を返す**（2026-09-20。不具合報告 ⑰。
ユーザー指摘「シミュレーションだとしても −100 もいくかな？」）。
それ以前は**曲線の最小値**だった。実測なら暗騒音が床を作るのでそれでよいが、
**シミュレーションには暗騒音が無い**ので、シュレーダー積分は応答の終端で
ゼロに向かい、**float64 の精度限界（−350 dB 前後）まで落ちるだけ**になる。
実案件（階段教室・63 Hz）では **−80 dB までは 3.5 秒かけて素直に落ちるのに、
そこから 1 秒足らずで −300 dB まで垂直に落ちていた**（最後のパルスの到来が 4.11 s）。

いまは**傾きが急変する点**を探す（`_decay_floor_db`）。

- 基準は「0 → `FLOOR_REFERENCE_DB`（−20 dB）まで落ちるのに要した平均の傾き」。
  その `FLOOR_SLOPE_FACTOR`（5）倍を超えた**最初の**点を崖の入口とする
- ★**「最後に緩やかだった点」を探してはいけない。**崖の先は数値の底で
  **平らになる**（傾き 0）ので、後ろから探すと底を拾ってしまう
- ★**基準の深さより手前は見ない**（立ち上がりの急降下を崖と取り違えないため）
- ★**崖が無ければ従来どおり最小値**（解析的な直線の減衰では意味が変わらない）
- ★★**副産物**：上の「ISO 3382 の余裕」の見張りはこの値を使うので、
  **最小値のままでは一度も出なかった**（−347 dB は常に −45 dB より下）
...
★★**図（PNG）は「いまの条件のもの」だけ消す**（`owns_figure`。2026-09-18。
不具合報告 ⑭）。それまでは図フォルダの PNG を**名前も見ずに全部消していた**ので、
条件ごとの CSV は残るのに**図だけ最後に回した条件のものしか残らなかった**
（一括実行では途中の条件の図が全滅する。実案件の階段教室で、条件を 1 つ
回すたびに前の条件の図が 110 枚消えた）。

ファイル名の頭を付けるようにした 2026-08-21 の時点で「古い条件のファイルが
混ざる」問題は**名前で解決している**ので、まとめて消す必要はもう無い。
...
判定は結果 CSV 側（`name_candidates`）と同じ「対象室名＋条件名」の考え方。
テストは `test_project()`（実案件と同じ 121 枚を並べて、条件1 の掃除で
11 枚だけが消えることを確認している）。
...
★★**「室の諸元」は 2026-09-18 に足した**（ユーザー要望「容積も行を足しておいて」）。
統計残響式は容積で決まる（`T = 0.161 V / A`）のに、**容積が結果のどこにも
残っていなかった**ので、理論値を後から検算できなかった
（実案件で諸元を別に作るはめになった）。**理論値の直前**に置いて、
表を上から ᾱ → A → V → T と読めるようにしてある。
値は `statistical` が最初から持っているもので、計算は増えない。

検算（テスト）：表の `容積_m3` と `等価吸音面積_m2` から `0.161 V / A` を出すと
`sabine_s` に一致する。★定数は丸めた 0.161 ではなく**音速から出る 24ln10/c**
（20℃ で 0.1607）なので 0.2% ほどずれる。
...
★**レイヤは一括で表示・非表示できる**（2026-09-09。ユーザー要望
「レイヤー全選択全解除ほしい」。不具合報告 WIN240377 の ⑦）。
実案件の階段教室は**画層が 22 種**あり、1 つずつでは手数が多すぎた。

- 左パネルの「レイヤを全表示」「レイヤを全非表示」「この番号のレイヤだけ表示」
- ★★**数字キーは 1〜9 まで**（VTK のキーイベントは 1 文字なので増やせない）。
  **10 個目以降は「レイヤ番号」の欄**（スピンボックス）＋
  「この番号のレイヤを選択に足す」で指す
- ★**コールバックを呼ぶだけではチェックの四角が変わらない**
  （VTK のチェックボックスは自分で状態を持つ）。`_set_layer_visible` が
  `SetState` も併せて呼ぶ
...
★2026-09-18 に消す相手を「いまの条件の図だけ」に絞った（不具合報告 ⑭）が、
**手で撮った画像は対象室＋条件名の頭が付いていないので巻き添えのままになる**。
子フォルダに分けておく必要は変わらない。
...
| 条件名 | **条件表のシート名**。優先は ①明示したシート → ②条件表のファイル名（既定名は除く） → ③**先頭シート名** | `Project.condition_label` |

    研修室.dxf ＋ 条件表.xlsx のシート「吸音追加案」 → 研修室_吸音追加案_rt.csv

★★**シートを選ばなくても条件名が付く**（2026-09-06 ユーザー判断。
不具合報告 WIN240377 の ②）。`condition_table.sheet_of()` は指定が無ければ
**条件表の先頭シート**を使うので、**材料は実際にそのシートのものが使われている**。
...
- ★★**実行の頭で `run_project._freeze_condition_name()` が焼き付ける**。
  条件表は計算の**途中**で作られる（`_update_condition_table` は面の情報が要るので
  `process` のあと）ので、焼き付けないと**同じ 1 回の実行の中で名前が変わる**
  （`室_吸音率と理論値.csv` と `室_現状_まとめ_….csv` が混在した。実際に踏んだ）
- ★**条件表がまだ無いときは `ct.FIRST_SHEET`（`現状`）を先取りする**。
  しないと「1 回目は名前なし・2 回目から名前あり」になり、
  **描き直し（`--redraw`）やまとめ表の作り直しで名前が変わる**
- ★**`project.json` には書き戻さない**（`save()` のあとに立てる）。
  利用者が選んでいないシート名を設定として残さないため
- `Project._fallback_sheet()` は**その Project の間は答えを凍結する**
  （途中で条件表ができても名前が動かないように）
- まとめ表と Excel は `RESULT_FILES` に無く `clear_results()` の対象外なので、
  書いたあとに **`Project.drop_old_names()`** で昔の名前を消す
  （残ると結果フォルダに並んでどちらが最新か分からなくなる）
- ★**計算済みの結果を読むときは必ず `name_candidates()` を通す**。
  `inverse_square.read_levels()` だけ `prefixed()` で 1 通りしか組んでおらず、
  **「先に計算してください」で止まっていた**（2026-09-06 に修正）
...
★★**「作成」（`create()`）はプロジェクトフォルダ直下に固定**（2026-09-09。
ユーザー要望「条件表の作成は、選択したプロジェクトフォルダ内にしてほしい」。
不具合報告 ⑥）。それまでは `Project.condition_path`（＝画面の「条件表（xlsx）」の欄）
を対象にしていたので、**欄に前の案件の表が残っていると、新しいプロジェクトには
何も作られず、その別案件の表が `update()` で書き換えられていた**。
`update()` は材料番号と安全率は守るが**面数・面積・参考の式は書き直す**ので、
気づかないまま他案件の入力ファイルが変わる（実害あり）。

- ★「参照…」で選んだ既存の表を使うのは**読むとき**の話。「作成」は別の操作
...
プロジェクトフォルダ/条件表.xlsx
  ...
  シート「現状」        ← **条件 1 つ ＝ シート 1 枚。シート名が条件名**。30 行の枠
    番号,区分,レイヤー名,材料番号,安全率,材料名（参考）,面数,面数（パッチ）,面積_m2
     1 ,レイヤ,01__研修室_壁_扉,1,,=IFERROR(VLOOKUP($D2,吸音率!$A$2:$B$1001,2,FALSE),""),2,1,3.3
...
モデルを作った人から「面数がやけに多い」と指摘された。数え方が違うだけだった。

| 列 | 中身 | 出どころ | 階段教室 |
|---|---|---|---:|
| `面数` | **三角形に割ったあとの枚数**。n 角形は必ず n−2 枚になるので**必ず増える** | `model.layer_counts` | 779 |
| `面数（パッチ）` | **計算が 1 枚として扱う数**（同一平面パッチ）。モデル作成者が数える面数に近い | `condition_table.layer_patch_counts()` | 223 |
| （参考）他ソフトの面数 | — | — | 241 |

- ★**数え方は本番と同じ呼び方**にする（`mesh_method.coplanar_patches()` に
  **材料も渡す**）。ここだけ別の数え方にすると、表の値が計算の実体とずれる
- ★**同じパッチが 2 つのレイヤにまたがることがある**（同一平面で隣り合い、
  材料も同じとき）。そのときは両方に数えるので、**合計は室全体のパッチ数より
  多くなりうる**。レイヤごとに並べる表なので、これが素直な数え方
- ★**どちらも参考列**。読むときは見ない（入力は「材料番号」と「安全率」だけ）
- ★**数えられなければ列を空のままにする**（0 と書くと「面が無い」と誤読される）。
  理由は黙って捨てずに出す
- 半数の画層はパッチ数が**ぴたり一致**した（反射板 42・床_階段 24・天井ボード 8）。
  こちらが少ない側は**まとめた分**（`PHP_後壁` は 6 面 → 1 枚 23.84 m²）で、
  まとめるのが正しい（三角形単位だと「同じ壁の隣の三角形に当たった」だけで
  バックトレースが経路を却下する）
...
★★**掛けた表は `procedure.process(absorption_table=…)` で本計算へ渡す**
（2026-09-20。不具合報告 ⑱）。それ以前は `procedure` が `material_library` から
**自前に組み立て直していた**ので、`run_project._absorption_table_for()` で掛けた
安全率が**本計算に届いていなかった**（`_model_for()` が作るモデルは
**経路キャッシュの指紋づくりと作図チェックにしか使われない**）。
実案件で条件を複製して安全率を入れて回したところ、パルス列のエネルギーも
吸音率と理論値も平均吸音率も**1 ビットも変わらなかった**。警告も出ない。
**吸音を見過ぎる＝危険側**に外れるので実害がある。

- ★`procedure` は**出来あいの表を渡されたら組み立て直さない**。
  これで指紋づくり・作図チェックと本計算が**同じ表**を見る
- ★**材料一覧が無いのに安全率が書いてあるときは警告する**（黙って捨てない）
- ★`read_dxffile` は `Mesh.material` に**レイヤ名**を入れる（材料名ではない）ので、
  **安全率の有無でパッチの切れ目は動かない**＝経路キャッシュは無効にならない
...
足りない列（安全率・材料名・**面数（パッチ）**）は**空いている列に足す**
（間に挿し込むと既にある `VLOOKUP($D2,…)` の参照がずれるため）。
★置き場は**見出しが埋まっている間は右へ送り、最初の空き列**（`_ensure_columns`）。
離れた場所にある利用者の列を飛び越しはしないが、**空いている列にしか書かない**
ので、前からある表に足しても中身は壊れない。
★これがあるので、**前からあるプロジェクトの条件表も次の計算で列が増える**
（作り直さなくてよい）。
...
### 音源が複数あるとき ― 1 点ずつ回して、見方を選ぶ（⑨。2026-09-15）

**きっかけ**：不具合報告 ⑨。`run_project._source_of()` が `model.source_points[0]` を
返し、`procedure.process()` の引数が単数だったため、**音源を 2 点描いても
2 点目が黙って捨てられていた**。警告も出ないので、実案件（階段教室）では
3 回・延べ 4 時間ぶんの計算がすべて S1 だけの結果になっていた。
...
#### 置き場
    ...
    結果/                      ← 音源にも受音点にも依らない（吸音率と理論値・測定点・開いた辺）
      src1/                    ← 音源 1（音線軌跡はここ。音源に依るので）
        rec1/ rec2/ …          ← 受音点ごと（中身は従来と同じ）
        <室>_<条件>_まとめ_*.csv ＋ 結果一式.xlsx
      src2/ …
      合成_平均/                ← 見方（`source_mix.py` が作る）
      合成_重ね合わせ/
      合成_重ね合わせ_時間差なし/

★**音源が 1 点だけのときは `srcM` の段を作らない**（`結果/rec1/` のまま）。
点が 1 つしかないのに深くすると、それまでのプロジェクトと置き方が変わる。

| 結果 | 受音点に依る | 音源に依る | 置き場 |
|---|---|---|---|
| `pulses` / `ir` / `rt` / `decay` / `clarity` / `spl` / `sti` / `paths` | ○ | ○ | `結果/srcM/recN/` |
| `raylog`（音線軌跡） | — | ○ | `結果/srcM/` |
| `room`（吸音率と理論値）/ `points`（測定点）/ `open_edges` | — | — | `結果/` 直下 |

`pj.SOURCE_SHARED_RESULTS` が 3 つ目の区分。`Project.result_dir(shared=, source_shared=)`
が振り分ける。まとめ表と Excel も棚ごとに作る（`summary.results_root()`）。

#### 3 つの「見方」の違い（★ここが判断の分かれ目）

| 見方 | 何を表すか | 干渉 | 作り方 |
|---|---|---|---|
| **平均**（`合成_平均`） | 音源位置ごとに測って平均する **ISO 3382 の作法** | 起きない | 音源ごとの指標を平均（音圧レベルだけ**エネルギー平均**） |
| **重ね合わせ**（時間差あり） | **2 台を同時に鳴らした**ときの音（PA など） | 起きる | パルス列を**絶対時刻のまま**1 本にまとめてから合成 |
| **重ね合わせ_時間差なし** | 伝搬距離の違いによる到達時刻のずれを消した形 | 起きる | 音源ごとに応答を作り、**波形をずらして**足す |

★**平均のとり方は行ごとに決める**（`_average_rows(tables, energy=_energy_columns(...))`）。
`spl.csv` は区分付きの表で dB でない行（`音源距離_m`）も混じるため、
**項目名が `_dB` で終わる行だけ**をエネルギー平均にする。

★★2026-09-18 に**行ごとに指定できる形へ直した**（不具合報告 ⑮）。
それまでは表を丸ごと 2 通り（算術・エネルギー）平均してから dB の行だけ
差し替えていたので、**見出しの行（`63, 125, …, 8000`）まで数値として平均**していた。
エネルギー平均では `10^(8000/10)` がオーバーフローして見出しが `inf` になり、
音源が複数のプロジェクトでは**毎回 `RuntimeWarning` が出ていた**
（差し替えで捨てられるので出力は正しかったが、本物の異常が埋もれる）。
実データ 220 表で**直す前と 1 文字も変わらない**ことを確認済み。

★★**時刻をずらしたパルス列をまとめて合成してはいけない。**
`impulse.impulse_response` は**到来時刻から伝搬距離を出す**
（`振幅 = √E /(t·c)`）ので、時刻を 0 に寄せると距離 0 になって発散する
（2026-09-15 に実際に踏んだ。残響時間の表が全部空になった）。
音源ごとに**正しい時刻のまま**合成してから、`np.roll` で波形をずらして足す
（`source_mix.aligned_impulse`）。
★**音圧レベル・STI はパルス列から出すが、`sound_level` は距離を別引数で受ける**ので、
時刻だけずらしたパルス列を渡してよい。

検算（テスト [50]）:
・同じパルス列を 2 つ重ねると音圧レベルが **+3.010 dB**（エネルギー和）
・`test.dxf` 改（音源 2 点）で、平均 6.868 dB / 重ね合わせ 9.879 dB
  ＝ 平均 + 3.01 dB（音源が 2 つなので合う）

★**音源ごとに PWL を変えることはできない**。パルスのエネルギーは
「音源パワー 1 あたり」の値で、`Lw` は最後に 1 回足すため
（＝同じ PWL の音源が複数、という扱い）。重み付きは TODO F-7b の続き。

★**`project.json` に音源を書き戻さない**（`_run_sources` は `save_settings=False`）。
書き戻すと `source` が 1 点に固定され、次回から 1 点目しか回らない
...
### ★★周波数バンドは「呼び値」と「厳密値」を分ける（2026-09-16）

ユーザー指示「厳密な中心周波数のほうに合わせてほしい」による。
根拠は**手元にある規格の原本**（`06_参考文献/02_規格・法律/`）で確認した。

| | 出どころ |
|---|---|
| **IEC 61260-1:2014 / JIS C 1513-1:2020** | オクターブバンド及び 1/N オクターブバンドフィルタ — 仕様 |
| **ISO 9613-1:1993** | 大気吸収（表 1 は**厳密中心周波数**で計算されている。注5） |

#### 使い分け

| | 何に使うか | 値 |
|---|---|---|
| **呼び中心周波数**（3.6） | **表・図・ファイル名・列見出し** | 63 / 125 / …、160 / 315 / 630 |
| **厳密中心周波数**（5.4.1 式(2)） | **計算に使う周波数**（空気吸収・帯域分割） | `f_m = f_r·G^(x/b)` |

- 基準周波数 `f_r` は**ちょうど 1000 Hz**（5.3）
- オクターブ周波数比 `G = 10^(3/10) = 1.995 26`（5.2.1 式(1)）＝**ベース 10**
- 変換は `absorption.exact_midband()`。★**厳密値を入れ直しても動かない**
  （いちばん近い格子点に丸めるだけ）ので、二重に掛けても安全

#### 帯域端（5.6.1 式(4)(5)）

    f1 = f_m·G^(-1/(2b))      f2 = f_m·G^(+1/(2b))

置き場は **`absorption.band_edges()` ただ 1 か所**。フィルタ（`impulse` /
`reverberation`）も吸音率の帯域平均（`absorption_curve`）もここを通す。
★**幅が食い違うと「その帯域の値」の意味がずれる**ので、別々に書かないこと。

| | 規格（ベース10） | 以前（ベース2） | 差 |
|---|---|---|---|
| オクターブ `f2/f_m` | **1.412 538** | √2 = 1.414 214 | 0.12% |
| 1/3 `f2/f_m` | **1.122 018** | 2^(1/6) = 1.122 462 | 0.04% |
| 1/3 `f1/f_m` | **0.891 251** | 2^(-1/6) = 0.890 899 | 0.04% |

★1/3 の 0.891 25 / 1.122 02 は**規格 表 F.1 の不連続点の値そのもの**。

#### 効き方

- **空気吸収が ISO 9613-1 表 1 と一致するようになった**。呼び値 8 kHz を
  そのまま渡すと 130.0 dB/km、厳密値 7943.3 Hz なら **128.0 dB/km**（規格どおり）
- **室内音響指標への影響はごく小さい**。既知の指数減衰を入れた検算で
  T30 の誤差が 0.1〜0.3 ポイント動くだけだった（**数字を直す変更ではなく、
  規格に沿って説明できるようにする変更**）
- ★**呼び値は変えていない**ので、CSV の列名・図・報告書の数字はそのまま
...
CAD のラベル（R1）と一致するとは限らない。実案件（階段教室）で
**`rec1` が R4** になっていた。

> 基本は作成順で良いですが、GUI 上で修正できるようにできないかな？（2026-09-15）

`プロジェクトフォルダ/測定点順.json` に**並び（DXF の何番目か）だけ**を持つ。
★**入力**なので対象室名・条件名の頭は付けない（`視点.json` `断面.json` と同じ）。
★**座標は持たない**。`normals.json` と同じく**点の数を控えて、合わなければ使わない**
（番号が抜けている・重複している並びも使わない）。黙って取り違えるほうが危ない。

当てるのは**モデルを読んだ直後に 1 回だけ**（`run_project._ordered()` /
`face_editor.load_model_for()`）。ここを通ったあとは、どのモジュールから見ても
「1 番目の受音点」は利用者が決めた 1 番目になる。レイヤ名（測線）も一緒に動く。

画面は設定画面の**「測定点の並び…」**（上へ・下へ・CAD の順に戻す）。
端末からは `python point_order.py <プロジェクト> --edit`、
`--reset` で `測定点順.json` を消して作成順に戻す。
★**次の計算から効く**（計算済みの `結果/recN/` は並べ替えない）。

#### ★★DXF を作り直すと並びが変わる（2026-09-20。不具合報告 ⑯）

実案件で**画層名を 1 つ変えただけ**の DXF に差し替えたら、三角形は 779 枚とも
頂点の並び順まで完全に同じなのに `POINT` の出てくる順が変わり、
**src1 が S2 に、rec3 が R5 に**なっていた。黙って番号が振り直されるので
結果を見るまで気づけず、**経路キャッシュの指紋も合わなくなる**
（実案件で 50 分回してから気づいて中止。最後まで行けば 2.5 時間）。

三方から手当てした。

1. **前回と突き合わせて知らせる**（`point_order.check_against_previous()`）。
   前回の `結果/<室>_測定点.csv` と座標を突き合わせ、**集合は同じで順番だけ違う**
   ときに、何がどう入れ替わったかと戻し方を出す。
   `run_project._ordered()` が**モデルを読んだ直後に 1 回だけ**呼ぶ
   - ★**位置そのものが変わっているときは何も言わない**（動かしたなら当然）
   - ★**`測定点順.json` は書き換えない**（知らせるだけ）。直すのは利用者の判断
   - ★返す並びは**必ず DXF に出てきた順の番号**。すでに `測定点順.json` が
     あるときは**合成して**返す（当たっている分は挙げない）
   - 戻すのは `python point_order.py <プロジェクト> --match-previous`
2. **経路キャッシュの理由に対処法を添える**（`path_cache.compare(points=…)`）。
   保存した位置が**いまの別の測定点**と一致するなら
   「★保存した位置はいまの音源の 2 番目です。順番が入れ替わっているだけかも」と出す
3. **変換の出口で並びを決め打ちにする**（`dxf_faces.sort_points()`）。
   `POINT` を**画層 → 座標**の順に並べ直すので、元の図面の `ENTITIES` の並びに
   依らず**何度変換しても同じ番号**になる。並べ直したことは実行ログに出す

---

### RTany ― 減衰曲線をどこで読むかを設計者が決める（2026-09-15）

**きっかけ**：階段教室の減衰が**二段階**（前半が速く減り、後半が遅く残る）で、
曲率 C が 125〜4k Hz で 10% を超えていた。T30 を 1 本の直線で読むこと自体に無理がある。
...
`reverberation.measures_with_any(start_db, end_db)` が `DECAY_MEASURES`
（EDT / T20 / T30）に **RTany** を足した組を返す。**置き換えではない**——
自動の数字と見比べられないと、どこを読むべきかの判断ができないため。

| 設定 | 既定 | 意味 |
|---|---|---|
| `rt_any_start_db` | -5 | RTany の開始 [dB]。空なら RTany を出さない |
| `rt_any_end_db` | -35 | RTany の終了 [dB]。前半だけ読むなら -15 など |
| `decay_fit` | `least_squares` | ISO 3382 の回帰／`crossing`（2 点法） |

★既定を **T30 と同じ区間**にしてあるのは、「まだ何も決めていない」ことが
数字を見れば分かるようにするため（T30 と同じ値が出る）。

★**どの区間で読んだかを `rt.csv` に残す**（`RTany_start_db` / `RTany_end_db` の行）。
利用者が決める値なので、表だけ見て分からないと後から読み返せない。
非線形性 ξ も他の指標と同じように出るので、**その区間が読めるかどうかも数値で残る**
（短い区間を取ると ξ は大きくなる。読んでよいかの判断材料）。

図（`plots.reverberation`）では **RTany だけ破線**（自動の 3 本と区別する）。
まとめ表（`summary.REVERBERATION_ROWS`）にも並ぶ。

測定側の `sti_measurement` プロジェクトが同じ考え方の `RTany` を持っている
（そちらは 2 点法。こちらは既定が ISO 3382 の回帰で、`decay_fit` で切り替えられる）。

---

### LayerControls ― レイヤの一括操作を画面で共用する（2026-09-15）

2026-09-06（不具合報告 ⑦「レイヤー全選択全解除ほしい」）で**面の確認画面にだけ**
入れていたため、**結果の画面（音線・音粒子・虚音源・音圧分布）に無かった**
（2026-09-15 ユーザー指摘）。実案件の階段教室は画層が 22 種あるので、
結果画面で「床だけ見る」に 21 回のクリックが要る状態だった。

`view_model_gui.LayerControls` に 1 つだけ置き、

- `face_editor`（面の確認）
- `view_model_gui.build_plotter`（結果・虚音源・音圧分布が共用）

の**両方が同じ部品を使う**。同じものを 2 つ持つと片方だけ直る状態が続くため。

    controls = LayerControls(panel, names, notice=…, on_pick=…)
    for name in names:
        controls.add(panel.checkbox(name, True, callback), callback)

★**ボタンはチェックボックスより先に作る**（パネルは作った順に積むので、
後にするとレイヤの数だけ下に押し下げられ、ページを送らないと見えない）。
★**`SetState` も呼ぶ**（コールバックだけではチェックの四角が変わらない）。
★**数字キーは 1〜9 まで**なので、10 個目以降は「レイヤ番号」の欄で指す。

`face_editor.layer_boxes` は共通部品への**窓**（property）にしてある。
昔からの呼び名で書かれた側（数字キーの処理・試験）をそのまま動かすため。
...
### ★★モデル（DXF）は 1 回だけ読んで使い回す（2026-09-20。高速化の提案 ①）

実案件（階段教室・779 三角形）は `read_model` 1 回に **1.74 秒**かかるのに、
**1 条件を回すのに 17 回**読んでいた（音源 × 受音点ごとに 1 回ずつなど）。
F-6「音線追跡は受音点をまたいで 1 回で済ませる」と同じ考えが、
モデルの読み込みにもそのまま当てはまる。

**重い工程はすべて材料に依らない。**

| 工程 | 吸音率の値に依るか | 材料の分け方に依るか |
|---|---|---|
| DXF の解析・三角形分割・法線・同一平面グループ | × | × |
| 容積・表面積・開いた辺・レイヤ別面積 | × | × |
| `Mesh.absorption_coefficient` | **○** | ○ |
| `Mesh.material` | × | ○ |

`read_dxffile.apply_absorption(model, table)` が**吸音率だけ貼り直す**。

- ★**`Mesh.material` は触らない。**そこにはレイヤ名（面ごとの指定があれば
  その材料名）が入っていて、**吸音率の値では変わらない**。
  だから**パッチの切れ目も動かない**＝経路キャッシュは無効にならない
- ★**バンド数はいま貼ってある吸音率の長さから決める**
  （既定 8 で決め打ちすると 6 バンドで回しているときに列がずれる）
- ★**`layer_materials` は作り直す**（『吸音率と理論値.csv』の材料別の集計がここから決まる）

使い回してよいかの鍵は `run_project._geometry_key()`。

    DXF のパスと更新時刻・大きさ／単位／法線の扱い／バンド数／
    normals.json ／ materials.json ／ 測定点順.json

★**吸音率の値は鍵に入れない**（F-9 の `path_cache` と同じ約束）ので、
**条件をまたいでも使い回せる**（材料を変えた 2 条件目は読み込み 0 回）。

- ★**並べ替えてから控える**（`po.apply` はその場で入れ替えるので、
  控えたあとにもう一度当てると二重に並べ替わる）
- ★`procedure.process(model=…)` に渡す。**このプロシージャはモデルを書き換えない**
  （読むだけ）ので、受音点をまたいで同じものを渡してよい
- ★`_face_count()`（面数の照合）には**別の控え**が要る。
  `_geometry_key` がここを呼ぶので `_model_for` の控えを使うと堂々巡りになる
- 切りたいときは `run_project.REUSE_MODEL = False`、捨てるときは `clear_model_cache()`

実測（実案件の DXF・音源 2 点 × 受音点 5 点）:
...
★**結果は 1 バイトも変わらない**（`結果/` 以下の CSV を全ファイル突き合わせ）。

### ★条件の一括実行は音源ごとの棚も回す（2026-09-20）

不具合報告 ⑪ ⑫ と同型で、最後に 1 か所だけ残っていたもの
（2026-09-16 の申し送り「`run_conditions()` の Excel 作り直しは棚を選ばない」）。

音源が 2 点以上あると結果は `結果/srcM/recN/` に入るのに、
**条件の比較表（`summary.write_condition_summary`）と条件ごとの Excel** だけ
`結果/` 直下を見ていたので、「条件ごとの結果がまだありません」で終わっていた。

`project.source_folders()`（`src1` `src2` `合成_平均`…）を回し、
**棚ごとに比較表と Excel を作る**。音源が 1 点なら棚は空なので従来どおり 1 回。
組み直しは `source_mix.tagged()` に通す（`pj.DEFAULTS` に `source_tag` は
入らないので、素直に組み直すと棚が落ちる）。

### ★変換の三角形分割も「いちばん形の良い耳」を切る（2026-09-20）

`read_dxffile._ear_clip` は前から「最小角がいちばん大きい耳」を切っていたが、
**`dxf_faces.ear_clip_2d` は「最初の耳」を切っていた**ので、
変換した DXF にだけスリバーが並んでいた（実案件で最小角 1° 未満が 113 枚）。

★**枚数は変わらない**（n 角形は n-2 枚が理論的な最小）。変わるのは**形**だけ。
実測（`床_1F` に近い外周 13 点＋穴 4 点のドーナツ）:

| | 三角形 | 面積 | 最小角の中央値 | いちばん悪いもの |
|---|---:|---:|---:|---:|
| 最初の耳（従来） | 17 枚 | 430.00 m² | 18.43° | 5.82° |
| **いちばん形の良い耳** | 17 枚 | 430.00 m² | **23.96°** | **14.47°** |

あわせて**面積の無い三角形を書き出さない**ようにした（`DEGENERATE_AREA = 1e-9`）。
キーホール法は**橋の両端を 2 度使う**ので橋の根元にできる。落としても面積が
無いので形は変わらず（面積の照合にも影響しない）、残すと法線が決まらない
（`read_dxffile.face_normal` が None を返し `skipped["縮退面"]` に落ちる）。

★**スリバーそのものは落とさない。**実案件の 113 枚は**面積が 32.2 m²**
（全体の 1%）あるので、消すと本当に穴が開く。


### ★★受音点ごとの子は `project.json` を書かない（2026-09-20。不具合報告 ⑲ ⑳）

`_run_one` に来る `project` は**受音点ごとの子**（`_sub_project`）で、親と同じ
`project.json` を指している。以前はその最後で

- **使った音源を `project.source` に書き戻して保存** → 次の実行で `_sources()` が
  それを DXF より優先し、**CAD で音源を動かしても古い位置のまま回った**（⑲）
- **子の `head_azimuth`（数値）で保存** → 親の**リストが 1 点ぶんに潰れた**（⑳）

設定は親の `run()` が頭で 1 回だけ保存する。`_run_one` と `_run_source` から
`write_back` / `save_settings` の引数を外した（受け渡す先が無くなったため）。
`run_conditions` の音源の書き戻しも外した。

★**過去の実行で書かれた値は残り続ける**ので、`run()` の頭で
`_warn_stale_points()` が `project.json` の音源・受音点を DXF の点と突き合わせ、
どれとも一致しなければ知らせる（勝手に消さない。同じ食い違いは 1 回だけ）。

### ★★吸音率の各段を結果に載せる（2026-09-20。不具合報告 ㉑）

『吸音率と理論値.csv』の「材料別の吸音率」は**統計式が使う乱入射の値**で、
`reverberation.surface_summary` が**モデルの垂直入射値を Paris の式で戻したもの**。
つまり**安全率と上限（0.951）の丸めを通したあと**で、カタログ値そのものではない。

すぐ下に区分を足した（渡されたものだけ。無ければ従来どおり）。

| 区分 | 中身 | 出どころ |
|---|---|---|
| 材料別の吸音率（カタログ値） | 吸音率シートの値。項目名に種類 | `condition_table.absorption_stages` |
| 安全率 | 掛けたレイヤだけ。値は 3 列目 | 同上 |
| 材料別の吸音率（垂直入射） | ★**音線追跡が実際に使った値** | `procedure` がモデルの面から取る |
| 上限に丸めた帯域 | 丸めた帯域だけに丸める前の値 | `absorption_stages` |

`read_room_csv` は「材料別の吸音率」しか見ないので既存の読み手は壊れない。

### ★★作図チェックの「作図ミス」は自由端だけで判定する（2026-09-20。不具合報告 ㉒）

`check_model` は開いた辺の**総数**（`open_edges` / `is_closed`）で「作図ミスです」と
判定していたので、**T字接合しか無い閉じた室でも出ていた**。自由端だけで判定し、
T字接合は情報として数だけ出すようにした。

★★**`model.free_edges` は開いていれば必ず数える**ようにした。以前は
**容積を出すとき（囲まれているとき）だけ**数えていたので、開いたモデルでは
既定値の `[]` のまま＝「数えていない」の意味になっていた。判定をそのまま寄せると
**開いたモデルに「面は閉じています」と言ってしまう**（test2.dxf：床＋壁 2 面で
`open_edges.py` は自由端 11 本と数えるのに `free_edges` は 0 だった）。
いまは `open_edges.collect` と同じ数になる。

### auralize.py ― 可聴化（2026-09-26 ユーザー要望）

> 計算したインパルス応答をドライソースに畳み込みをして，可聴化したい。
> 再生を続けながらも，畳み込む ON/OFF の切り替えや条件の切り替えが即座に効くように。
> 可能であれば，マイク入力を畳み込んでスピーカ等へ出力できる機能も。
> 元音源の波形を表示し，選択した場所から再生，選択した範囲をループ再生など。

`python auralize.py <プロジェクト>`（設定画面の「可聴化…」からも開く）。

**作り**：Python は `結果/` を読んで渡すだけの小さなサーバ（標準ライブラリの
`http.server`。**127.0.0.1 だけで待ち受ける**）。画面は `auralize.html` を
Edge / Chrome の**アプリ窓**（`--app=`）で開く。**追加のライブラリは入れていない**。
リアルタイムの畳み込み（分割 FFT）・マイク入力・出力先の切り替え・波形の描画が
ブラウザ（Web Audio）にそろっているため。

| 決めごと | 中身 | 理由 |
|---|---|---|
| ★★**開いたときは並べるだけ** | 中身は使うときに読む。T20/T30/EDT は画面に出ている分の `rt.csv`、応答は温める分だけ。読んだ応答は 48 本まで覚える | 実案件（階段教室）は応答 **1,040 本 × 7.4 MB ＝ 7.7 GB**、しかも OneDrive の共有フォルダで**クラウドにしか無い**。1 本 1 秒弱で落ちてくる |
| ★★**音量の比を保つ** | **並びの先頭の応答（最初の条件・src1・rec1）**を基準にした**共通の 1 つの係数**（`Catalog._gain`）だけを掛ける。カードの「音量」はその基準に対する dB。ブラウザ側も `disableNormalization: true` | 応答は `振幅 = √E/(t·c)` で絶対値を持つ。条件ごとに正規化すると「吸音を増やすと静かになる」「遠い受音点は小さい」が聞こえなくなる |
| ★**周波数の変換はサーバで** | ブラウザの出力周波数（多くは 48 kHz）へ `resample_poly`。★**`fs_in/fs_out` を掛ける** | `ConvolverNode` は再生と同じ周波数の応答しか受けない。畳み込みは「サンプルの和」なので、掛けないと 44.1→48 kHz で +0.74 dB 大きく出る（テスト [62]） |
| ★★**切り替えは「温めておいた畳み込み」の音量を渡すだけ** | 選んでいる受音点・音源位置の条件を最大 12 本、**入力を聴かせたまま**音量 0 で走らせておき、40 ms で渡す | 新しく作った畳み込みは過去の入力を知らないので、切り替えた瞬間に**響きの尾が途切れる**。温めておけば履歴を持ったまま替わる |
| 響きの ON/OFF | 元の音と響きを 40 ms でクロスフェード。畳み込みは止めない | 同上（ON に戻した瞬間から尾がある） |
| 聴き方 | 既定は**モノラル**（L+R）。ステレオのままも選べる | 応答は 1 ch（無指向の受音点）。点音源として聴くのが素直 |
| ★**ハウリング対策**（スピーカで鳴らすとき。2026-09-26） | マイクの欄で個別に ON/OFF。①**周波数シフト**（既定 +5 Hz。AudioWorklet でヒルベルト FIR 511 タップ＝5.3 ms の遅れ → `y = x·cos φ − H[x]·sin φ`。**マイクを畳み込んでいるときだけ**出力の手前に挟む）②**ゲート**（包絡がしきい値を下回って 0.25 s 待ってから 0.15 s で閉じる。入力の大きさとランプを出す）③**エコーキャンセル**（`getUserMedia` の `echoCancellation`。切り替えるとマイクをつなぎ直す）。雑音抑圧・自動音量はいつも切る | 実測（オフライン描画）：シフトの逆側の成分は 300 Hz で −79 dB・1 kHz で −107 dB。★**100 Hz では −17 dB**（FIR の長さの限界。長くすると遅れが増えるので 5 ms に留めた）。★**奇数番目のタップを回すと全部 0**（M = 255 が奇数なので係数があるのは偶数番目。最初これで踏んだ） |
| ★**マイクのまま一時停止** | 再生ボタン／Space で「停止 ⇄ 再開」。マイクはつないだまま、畳み込みへ送る音量だけ 0 にする。入力の波形とゲートの表示は動き続ける | 設定を確かめる間に止めたい（ユーザー要望） |
| 振り切れ防止 | 出力の直前にリミッタ（`DynamicsCompressor`、-1 dBFS・20:1）。外せる | マイク入力や響きの長い条件で耳を守る |
| 画面が閉じたら止まる | 画面が 5 秒ごとに `/api/ping`、閉じるときに `/api/bye`。★途絶えてから **150 秒**で止まる | 窓を最小化するとブラウザは定期処理を 1 分に 1 回まで間引くので、短くすると聴いている最中に止まる |

**置き場の読み方**：`結果/[srcM/]recN/<室>_<条件>_ir.csv`。条件は**ファイル名から室名の頭を外した残り**、
音源位置と受音点はフォルダ名。★**棚（`srcM` / `合成_…`）と `recN` 以外のフォルダには降りない**（`旧_単一音源S1のみ_260915` のような控えを拾わない）。条件は**数を数として**並べる（条件2 → 条件10）。カードの T20 / T30 / EDT は同じ場所の `rt.csv` の中音域（500・1k Hz）の平均、
小さな減衰曲線は全帯域のシュレーダー積分（応答を読んだらブラウザで描く）。

**波形の緑青の帯（室の響きの包絡）**は、元の音の列ごとの二乗平均を応答の二乗（エネルギー）で畳み込んだ
**見積もり**（位相を捨てた無相関の近似）。元の音はピークで描いているので、元の音の波高率を掛けて同じ物差しにしている。
拡大しすぎる（応答が 8000 列を超える）と重いので描かない。

**ドライソース**：★音声本体は Git 管理外（public のため）。一覧とコピー手順は `ドライソース/README.md`。リポジトリ直下の `ドライソース/`（共通）と、プロジェクト直下の `ドライソース/`（その案件だけ）を探す。
★**置けば並ぶ**：一覧は `/api/sources` のたびにフォルダを見直す（控えなし）ので、画面の「一覧を更新」だけで足りる（結果は読み直さない）。**サブフォルダも見る**（名前は `坂吉/声`）。
★**画面へ落とした音は共通の `ドライソース/` に保存**（`save_dry`。上書きしない：同じ中身ならそのまま、違えば `名前_2.wav`）。保存できなかったときだけ、その画面の間だけの「持ち込み」にする。
読み出し・保存とも `realpath` で**置き場の外へ出ない**ことを確かめる（テスト [62]）。

**まだ確かめていないこと**：マイク入力は Claude のプレビュー画面ではマイクが塞がれているので、実機（Edge）で要確認。
