...
`JRE\研修室.dxf` では、これを入れる前は音線 2000 本中 1952 本が室外へ逃げ、入れたら 3 本になった。
...
（`JRE\研修室.dxf` でパルス列 2298 本・バンド別エネルギー合計まで一致）。
...
**`'inward'`（`orient_inward()`）** は 2026-08-14 に、実務の CAD データ（`JRE\研修室.dxf`）で
...
2026-08-14 まで未対応で、該当する面をまるごと読み飛ばしていた（`JRE\研修室.dxf` で壁 19 枚が消えていた）。
