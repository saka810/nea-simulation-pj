...
| 決めごと | 中身 | 理由 |
|---|---|---|
| ★★**開いたときは並べるだけ** | 中身は使うときに読む。T30/EDT は画面に出ている分の `rt.csv`、応答は温める分だけ。読んだ応答は 48 本まで覚える | 実案件（階段教室）は応答 **1,040 本 × 7.4 MB ＝ 7.7 GB**、しかも OneDrive の共有フォルダで**クラウドにしか無い**。1 本 1 秒弱で落ちてくる |
...
音源位置と受音点はフォルダ名。★**棚（`srcM` / `合成_…`）と `recN` 以外のフォルダには降りない**（`旧_単一音源S1のみ_260915` のような控えを拾わない）。条件は**数を数として**並べる（条件2 → 条件10）。カードの T30 / EDT は同じ場所の `rt.csv` の中音域（500・1k Hz）の平均、
