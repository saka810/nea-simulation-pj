...
| `condition_table.py` | 材料条件表（レイヤー名 → 吸音材の CSV）と条件の探索 | （移植元なし・新規） | **実装済**（2026-08-21 新設） |
...
プロジェクトフォルダ/
  ...
  吸音追加案.csv        条件ごとに名前を付けて並べられる（**この名前が条件名**）
  結果/                 ← **受音点に依らないもの**は直下
...
- **スクロールは試したが取り下げた。** VTK のウィジェットはテキストと違い、
  置き直しを飛ばすと古い位置に残る。範囲外を隠す処理を入れても
  **チェックボックスが 4 行ぶんずれて文字に重なった**ので、原因を詰める前に引き返した
- 代わりに**載せる内容を削った**：「音線の情報」はコンソールに出しているのでパネルから外し、
  スライダのバー高さを 34 → 28 px に詰めた。**458 → 268 px** に減った
- それでも入りきらないときは**コンソールに操作説明を出す**（`hidden_height()` で判定）。
...
| 条件名 | **材料条件表のファイル名**（既定名 `材料条件表.csv` なら付けない） | `Project.condition_label` |

    研修室.dxf ＋ 吸音追加案.csv → 研修室_吸音追加案_rt.csv

条件表は条件入力ウィンドウの「材料条件表（CSV）」で選ぶ。
同じフォルダに `現状.csv` `吸音追加案.csv` … と並べておけば選び替えられる。
...
### condition_table.py ― 材料条件表（依頼 2026-08-21）

> 壁の吸音材条件を変える場合に、CAD データを変えて、それをまた読み込んでもらって、
> っていうのが大変。レイヤー名と壁材料の一覧表（条件表）を作って、
> それを読み込ませる形式に変えたい。

`プロジェクトフォルダ/材料条件表.csv` が**レイヤー名 → 吸音材の対応表**になる。
CAD のレイヤ名を書き換えずに材料を差し替えられる。
...
- **書き換えるのは「材料名」の列だけ。**空欄なら従来どおり
  レイヤー名（または先頭の番号）で引く
- 面数・面積・吸音率は**参考**で、計算のたびに書き直される
- `#` 行（対象室・モデル・吸音率表の種類）は読み飛ばす覚え書き
- 区分 `面ごとの指定` は `face_editor` で貼った材料の**記録**（入力ではない）

★**更新の約束：利用者が書いた材料名は絶対に上書きしない。**
`update()` は既存の表を読み、材料名はそのまま残して参考列だけ書き直す。
モデルから消えたレイヤは区分を `（モデルに無し）` にして残す
（消すと「前はどう設定していたか」が分からなくなる）。

置き場はプロジェクト直下で、**対象室＋条件名の頭は付けない**
（結果ではなく入力なので、`normals.json` と同じ扱い）。
入口は条件入力ウィンドウの「材料条件表を開く」と `python condition_table.py <フォルダ>`。
`project.json` の `assignment` より**条件表が優先**される。
