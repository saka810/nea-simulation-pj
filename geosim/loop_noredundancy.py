...
def loop(soundsource_point, reciever_point, absorption_list, mesh_id_list, mesh):
    # 初期条件　虚音源＝音源
    # imaginary_sourcepoint = np.zeros((nref, 3))
    imaginary_sourcepoint = np.zeros((mesh_id_list.shape[1], 3))
    ...
    for i in range(mesh_id_list.shape[0]):
        # 反射回数ループ
        # for k in range(nref):
        for k in range(mesh_id_list.shape[1]):
        ...
        for k in range(mesh_id_list.shape[1],0,-1):
